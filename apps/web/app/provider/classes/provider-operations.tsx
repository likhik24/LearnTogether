...
/** Class management wired in from the page so classes + their sessions render together. */
...
export function ProviderOperations({ classControls }: { classControls: ProviderClassControls }) {
  ...
  /** Upcoming sessions grouped by the class they belong to. */
  ...
  return (
    <section className="provider-operations" id="operations">
      ...
      <div className="provider-class-sessions">
        <div className="section-heading">
          <h3>Your classes &amp; sessions</h3>
          <span>{classes.length}</span>
        </div>
        {classes.map((item) => {
          const classSessions = upcomingByClass.get(item.id) ?? [];
          return (
              ...
              <div className="provider-class-session-rows">
                {classSessions.length > 0 ? (
                  classSessions.map((session) => (
                    <div className="provider-session-row" key={`${session.classId}-${session.originalStart}`}>
                      <div>
        ...
        {recent.map((session) => (
          <article className="provider-session-row" key={`recent-${session.classId}-${session.originalStart}`}>
            <div>
